...
# Sidebar: Question navigation with status indicators
with st.sidebar:
    st.header("Jump to Question")
    for idx, q in enumerate(QUESTIONS):
        user = st.session_state.user_answers[idx]
        correct = (user == q["answer"]) if user is not None else None
        if user is None:
            color = "#adb5bd"  # gray
            label = f"{idx+1}"
        elif correct:
            ...
            color = "#dc3545"  # red
            label = f"{idx+1} ✗"
        if st.button(label, key=f"nav_{idx}", help=q["question"], use_container_width=True):
            jump_to_question(idx)
...
if not st.session_state.finished:
    ...
    if st.button("Restart"):
        ...
        st.session_state.show_feedback = False
        st.session_state.last_correct = None
>>>>>>> 0df79cfd5b281e3262bafdbe8f83d16efddf2ad6
        st.session_state.last_explanation = "" 